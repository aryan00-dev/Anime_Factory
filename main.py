...
print("--- 🚀 VEDA CLOUD FACTORY ENGINE (100% COMPLETE & SECURE) STARTED ---")
...
if not all([API_KEY, META_ACCESS_TOKEN, IG_ACCOUNT_ID]):
    print("❌ CRITICAL ERROR: GitHub Secrets missing! Factory band ho rahi hai.")
    exit()
...
print("📥 Google Drive se raw episode utha raha hu...")
try:
...
try:
    ...
    print("⚠️ AI Error, backup data use kar rahe hain:", e)
    start_time, hook_text, detected_mood = 100, "Wait for it 🤯", "Action"

# --- 5. AUDIO DOWNLOADER ---
print(f"🎵 Mood '{detected_mood}' ke hisab se audio nikal raha hu...")
...
try:
    ...
    print("⚠️ Audio Download Warning:", e)
...
# --- 6. THE EDITOR ---
print("✂️ Kainchi, Audio aur Hacker Effects lag rahe hain...")
REEL_DURATION = 30
...
try:
    ...
    print("⚠️ Visual Layers skip huye (ImageMagick error ho sakta hai).")
...
print("🔥 Final Reel Render ho rahi hai...")
...
print("☁️ Meta API ke liye public link bana raha hu...")
...
if public_video_url:
    print(f"🔗 Public URL ready. Instagram par upload shuru...")
    ...
    if 'id' in res:
        creation_id = res['id']
        print(f"⏳ Container (ID: {creation_id}) ban gaya. Meta server rendering... 30 sec wait.")
        time.sleep(30) 
        ...
        if 'id' in pub_res:
            print("✅ BOOM! Reel successfully Instagram par POST ho gayi!")
        else:
            print("❌ Publish Failed:", pub_res)
    else:
        print("❌ Container Failed:", res)
else:
    print("❌ Public URL nahi ban paya.")
